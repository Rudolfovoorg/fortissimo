...
BASE_URL = "_ENERGYDATA_API_URL_"
BASE_URL2 = "_MEASUREMENTS_API_URL_"
API_KEY = "_API_KEY_"
headers = {
    "X-Api-Key": API_KEY
}
params = {
    "ClientId": 4,
    "from": datetime.now(tz=pytz.timezone("Europe/Ljubljana")).replace(minute=0,second=0,hour=0) - timedelta(minutes=288*20+30),
    "to": datetime.now(tz=pytz.timezone("Europe/Ljubljana")).replace(minute=0,second=0,hour=0)
}
...
def prep_energy(clientId):
    ...
    if response.status_code == 200:
        ...
        return
    data_all.columns = [
    col[:1].upper() + col[1:] for col in data_all.columns
    ]
...
def prep_weather(clientId):
    ...
    response = requests.get(BASE_URL2, params=params, headers=headers)
    df_w = pd.DataFrame()
    if response.status_code == 200:
        data = response.json()
        ...
        return
    df_w.columns = [
    col[:1].upper() + col[1:] for col in df_w.columns
    ]
    ...
    merged_15min_mean_w = df_w.resample('15min').mean()
...
def _load_tflite(model_path: str, num_threads: int = 1):
...
def _tflite_predict(interpreter, input_details, output_details, x: np.ndarray) -> np.ndarray:
    """
    x: np.ndarray, typically shape (1, SEQ_LEN, n_features)
    returns: np.ndarray output (e.g. shape (1, 96))
    """
    in0 = input_details[0]

    # If model input has fixed shape, x must match exactly.
    # If it supports resizing, we can resize.
    expected_shape = tuple(in0["shape"])
    if tuple(x.shape) != expected_shape:
        # allow dynamic batch / dynamic dims if supported
        interpreter.resize_tensor_input(in0["index"], x.shape)
        interpreter.allocate_tensors()
        # after reallocating, refresh details (indexes can change)
        input_details = interpreter.get_input_details()
        output_details = interpreter.get_output_details()
        in0 = input_details[0]

    x = x.astype(in0["dtype"])
    interpreter.set_tensor(in0["index"], x)
    interpreter.invoke()
    y = interpreter.get_tensor(output_details[0]["index"])
    return y


def forecast_next_day_tflite(clientId, df_full, tflite_threads: int = 1):

    MODEL_FILE = os.getenv("MODEL_PATH")
    SCALER_X_FILE = os.getenv("SCALERX_PATH")
    SCALER_Y_FILE = os.getenv("SCALERY_PATH")
    FEATURE_COLS_FILE = os.getenv("FEATURE_COLUMNS")

    FORECAST_FILE = "forecast_next_day.csv"

    SEQ_LEN = 288
    PRED_STEPS = 96

    interpreter, input_details, output_details = _load_tflite(MODEL_FILE, num_threads=tflite_threads)
    ...
    x_input = np.expand_dims(X_all_scaled[-SEQ_LEN:], axis=0)

    # TFLite inference
    y_next_scaled = _tflite_predict(interpreter, input_details, output_details, x_input)

    # y_next_scaled should be (1, 96) or (96,) depending on conversion
    y_next_scaled = np.asarray(y_next_scaled)
    y_next_scaled = y_next_scaled.reshape(-1, 1)

    y_next = scaler_y.inverse_transform(y_next_scaled).flatten()
...
def run_prediction_sequnece():
    energy_df = prep_energy(4)
    weather_df = prep_weather(4)
    ready_df= prep_features(data_energy=energy_df,data_weather=weather_df)
    TFLITE_THREADS = int(os.getenv("TFLITE_THREADS", "1"))
    return forecast_next_day_tflite(4,ready_df,TFLITE_THREADS)
#run_prediction_sequnece()
